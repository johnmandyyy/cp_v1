    ...
    def randomGen(self):
        ...
        while i < 1095:
            Symptom.objects.create(
                disease=Disease.objects.get(id=random.choice(list_of_ids)),
                warts=random_bool(),
            ...
            if len(result) > 0:
                for e in result:
                    inputs = [
                        ...
                        e.swelling_eyes,
                        e.weight_loss,
                        e.reduced_water_consumption,
                        e.diarrhea,
                        e.less_egg_production,
                        e.difficulty_breathing,
                        e.pale_comb,
                        e.nasal_discharge,
                        e.watery_eyes,
                        e.paralysis,
                    ...
                    Symptom.objects.filter(id=int(e.id)).update(
                        belongs_to="Validation Set",
                        forecasted_disease=int(predicted_value),
                        is_correct=remarks,
                    )
                ...
                iteration_testing = 0
                count_testing = 0
                for e in Symptom.objects.raw(
                    "SELECT * from app_symptom where belongs_to = 'Testing Set'"
                ):
                    inputs = [
                        e.warts,
                        e.loss_of_appetite,
                ...
                if len(Analysis.objects.all()) < 1:
                    Analysis.objects.create(
                        total_dataset=int(len(Symptom.objects.all())),
                ...
                print(
                    "Validation Accuracy: ",
                    (count_true / iteration) * 100,
                    "Testing Accuracy: ",
                    (count_testing / iteration_testing) * 100,
                )
        ...
        try:
            if len(ChickenHistory.objects.raw(eighty_query)) > 1:
                for each_rows in ChickenHistory.objects.raw(eighty_query):
                    Symptom.objects.filter(id=int(each_rows.id)).update(
                        belongs_to="Training Set"
                    )
...
class TestingSetListCreate(generics.ListCreateAPIView):
    ...
    queryset = Symptom.objects.raw(
        'SELECT "app_symptom"."id", "app_symptom"."disease_id", "app_symptom"."warts", "app_symptom"."loss_of_appetite", "app_symptom"."lesions", "app_symptom"."blister", "app_symptom"."swelling_eyes", "app_symptom"."weight_loss", "app_symptom"."reduced_water_consumption", "app_symptom"."diarrhea", "app_symptom"."less_egg_production", "app_symptom"."difficulty_breathing", "app_symptom"."pale_comb", "app_symptom"."nasal_discharge", "app_symptom"."watery_eyes", "app_symptom"."paralysis", "app_symptom"."watery_feces", "app_symptom"."belongs_to", (SELECT name from app_disease where app_disease.id = forecasted_disease_id) as forecasted_disease_id, "app_symptom"."is_correct", "app_disease"."id", "app_disease"."name", "app_disease"."description", "app_disease"."treatment" FROM "app_symptom" LEFT OUTER JOIN "app_disease" ON ("app_symptom"."forecasted_disease_id" = "app_disease"."id") where belongs_to = "Testing Set"'
    )
...
class CreateSymptomsListCreateView(generics.ListCreateAPIView):
    from .models import Symptom
    from .serializers import CreateSymptomsSerializer

    queryset = Symptom.objects.all()
    serializer_class = CreateSymptomsSerializer
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ["id"]
...
def setCurrentValues(id):
    ...
    # GET THE DATASET
    query = (
        'SELECT "app_chickenhistory"."id", "app_chickenhistory"."is_morning_bath", "app_chickenhistory"."is_afternoon_bath", "app_chickenhistory"."is_vitamin_a", "app_chickenhistory"."is_vitamin_d", "app_chickenhistory"."is_vitamin_e", "app_chickenhistory"."is_vitamin_k", "app_chickenhistory"."is_vitamin_b1", "app_chickens"."fowl_pox_vaccine" FROM "app_chickenhistory" INNER JOIN "app_chickens" ON ("app_chickenhistory"."chicken_id" = "app_chickens"."id") WHERE app_chickenhistory.id = '
        + "'"
        + str(id)
        + "'"
    )
...
def getPrediction(request):
    ...
    def predictValues(inputs):
        ...
        try:
            ...
            treatment = ""
            if len(Disease.objects.filter(id = int(final_val))) > 0:
                for each_rows in Disease.objects.filter(id = int(final_val)):
                    treatment = str(each_rows.treatment)


            for tree in loaded_regressor.estimators_:
                prediction = tree.predict(inp)[0]
                predictions.append(
                    getPredictedDisease(int(round(float(prediction), 6)))
                )
            ...
            return "Final Verdict: <br>" + str(fval) + " the treament is: " + treatment
        except Exception as e:
            return str(e)
